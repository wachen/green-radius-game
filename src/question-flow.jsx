...
// ─── golden celebration (one-time 60/60 full-board moment) ────────────────────
// Fires once per save when the camp's grand total hits a perfect 60/60 (see
// isPerfectTotal in src/core.jsx and the `goldenSeen` autosave flag in
// green-radius.jsx). Distinct from and bigger than the per-sector Celebration
// above: a gold-toned Fx burst plus more/larger splats, longer on screen, and
// full-board copy (no sector name). Honors prefers-reduced-motion with the
// same quiet text-only flash pattern.
function GoldenCelebration({ onDone }) {
  ...
  // Deterministic random per mount, same approach as Celebration — more/bigger
  // splats than the per-sector version, gold-toned.
  const splats = useMemo(() => {
