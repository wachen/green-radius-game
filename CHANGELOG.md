...
- Play first, details later (review item U5): the intro's camp name, lead, and email fields are now optional so anyone can start playing immediately; whoever skips them gets a "Get your results" card on the done screen with the same fields and consent, required before results are emailed or counted (#78)
- Double-submit guard (review item R4): each submission carries a persisted idempotency nonce, reused when a reload replays the POST; the Worker passes it to the sheet backend and uses it as the Resend Idempotency-Key so a replay can't send the result email twice (#77)
