...
| `worker/index.js`  | Cloudflare Worker — `POST /api/complete` + `POST /api/event` + `POST /api/client-error` (log-only beacons) + `GET /api/admin/responses` + `GET /api/health` + `GET /api/city` (public tally: aggregates plus the camp-map list of name + parsed playa address, nothing else per camp) + `GET /result/?r=` (per-camp OG unfurl); all else served as static assets |
