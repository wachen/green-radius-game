// @generated from src/fx.jsx by scripts/build.js — DO NOT EDIT.
...
function _fxDraw(ctx, p, a) {
  ...
  if (p.kind === "leaf") {
    ...
    ctx.beginPath();
    ctx.ellipse(0, 0, p.size, p.size * 0.48, 0, 0, FX_TAU);
    ctx.fill();
    ctx.restore();
    ...
    ctx.strokeStyle = p.color;
    ctx.lineWidth = Math.max(1, p.size * (1 - p.age / p.life));
    ctx.beginPath();
    ctx.arc(p.x, p.y, p.r, 0, FX_TAU);
    ctx.stroke();
  } else {
    ctx.fillStyle = p.color;
    ctx.beginPath();
    ctx.arc(p.x, p.y, p.size, 0, FX_TAU);
    ctx.fill();
  }
  ctx.globalAlpha = 1;
}
...
const Fx = {
  burst(x, y, spec) {
    if (prefersReducedMotion() || !_fx.ctx)
      return;
    const n = spec.n || 12;
    for (let i = 0;i < n; i++) {
      const ang = spec.angle == null ? Math.random() * FX_TAU : spec.angle + (Math.random() - 0.5) * (spec.spread || 1.2);
      ...
      _fx.ps.push({
        ...
        y,
        vx: Math.cos(ang) * sp,
        vy: Math.sin(ang) * sp - (spec.up || 0),
        g: spec.g == null ? 0.12 : spec.g,
        drag: spec.drag == null ? 0.99 : spec.drag,
        age: 0,
        life: (spec.life || 40) * (0.7 + Math.random() * 0.6),
        size: (spec.size || 5) * (0.6 + Math.random() * 0.8),
        rot: Math.random() * FX_TAU,
    ...
    if (_fx.ps.length > FX_CAP)
      _fx.ps.splice(0, _fx.ps.length - FX_CAP);
    _fxStart();
  },
  _center(el) {
  ...
  clear() {
    _fx.ps.length = 0;
    if (_fx.ctx)
      _fx.ctx.clearRect(0, 0, _fx.w, _fx.h);
  }
};
function FxLayer() {
