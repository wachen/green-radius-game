...
- **The game UI is nine babel scripts in one shared scope** (split from the old `green-radius.jsx` monolith in PR #55): `src/core.jsx` (hooks, constants, persistence, per-question scoring — loads **first**), `src/fx.jsx` (canvas particle FX), `src/badge.jsx` (`SectorIcon`, arc geometry, `RadialBadge`), `src/wheel.jsx`, `src/question-flow.jsx` (question modal, toast, celebration), `src/share-card.jsx` (PNG export, `ShareCard`, `ResultCardSVG`), `src/home.jsx` (FAQ modal, mode picker), `src/form-mode.jsx`, and `green-radius.jsx` (`GreenRadiusGame` + intro + done/email screen with the Green-Up Plan — loads **last**; its trailing `Object.assign(window, …)` expose reads the earlier modules). `index.html` loads all nine in that order; `result/`, `city/`, and `admin/` load only the subset they render (`core` + `badge`, plus `share-card` on `result/`) — `src/core.jsx` always first. Components reference each other by **bare name** within the shared Babel scope (see gotcha below).
...
- **Babel shared scope.** Every `<script type="text/babel">` runs in one shared scope, so components in the game scripts (`src/*.jsx`, `green-radius.jsx`) are referenced by **bare name** across babel scripts — they are **not** `window` properties. Only plain scripts that assign `window.X = …` create reliable globals (`window.SECTORS`, `window.ResultState`, `window.Rank`); `green-radius.jsx`'s trailing `Object.assign(window, …)` mirrors four component names, but only on pages that load it, and nothing in the repo reads them — always use the bare name. Mounting a component as `window.ShareCard` on a page that never loads `green-radius.jsx` (like `result/`) → `undefined` → renders nothing (this was a real bug).
