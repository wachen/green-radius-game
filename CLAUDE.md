...
- **The `greens` shape is the contract.** `{ food, water, waste, transport, shelter, power }`, each `0–4`, threads through game → `result-state` encode → Worker → sheet columns → email. Change it in one place and you must change it everywhere. **Invariant:** greens go green consecutively from Tier 1, so radius depth == green count — `result-state` stores only the *count* per sector, relying on this.
