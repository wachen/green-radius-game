...
The Green Radius Game — a self-ranking sustainability game for Burning Man theme camps, live at **https://greenradi.us**. It's a **no-bundler static app** (React 18 UMD served same-origin from `vendor/`; the game's `.jsx` sources are precompiled to classic-runtime JS under `dist/` by a small Bun script, `scripts/build.js`) fronted by one tiny Cloudflare Worker.
...
- **Entry:** `index.html` loads React + ReactDOM (vendored, same-origin, no CDN) plus the precompiled `dist/*.js` game scripts (classic `React.createElement` JS, built from the `.jsx` sources by `scripts/build.js` — no in-browser Babel), defines `PALETTE`, and mounts `<GreenRadiusGame variant="flat-playa" palette={PALETTE}/>`.
...
- **Three plain scripts create the only real globals:** `game-data.js` → `window.SECTORS` (6 sectors × 4 tiers of Yes/No content), `result-state.js` → `window.ResultState` (encode/decode a result to/from the result-link payload), and (retired in #66, kept served for cached pages) `rank.js` → `window.Rank`. `game-data.js` is isomorphic and is also imported by the Worker (the completion email's headline + Green-Up Plan).
