...
- **Entry:** `index.html` loads `beacon.js` (client-error beacon, first and un-deferred), the Preact runtime (core + hooks + compat + shim, vendored, same-origin, no CDN; the shim exposes `window.React`/`window.ReactDOM`), the precompiled `dist/*.js` game scripts (classic `React.createElement` JS, built from the `.jsx` sources by `scripts/build.js` — no in-browser Babel), and finally `dist/src/boot-index.js`, which defines `PALETTE` and mounts `<GreenRadiusGame variant="flat-playa" palette={PALETTE}/>`.
- **The game UI is nine compiled scripts in one shared global scope** (split from the old `green-radius.jsx` monolith in PR #55): `src/core.jsx` (hooks, constants, persistence, per-question scoring — loads **first**), `src/fx.jsx` (canvas particle FX), `src/badge.jsx` (`SectorIcon`, arc geometry, `RadialBadge`), `src/wheel.jsx`, `src/question-flow.jsx` (question modal, toast, celebration), `src/share-card.jsx` (PNG export, `ShareCard`, `ResultCardSVG`), `src/home.jsx` (FAQ modal, mode picker), `src/form-mode.jsx`, and `green-radius.jsx` (`GreenRadiusGame` + intro + done/email screen with the Green-Up Plan — loads **last**). Each compiles 1:1 to `dist/src/*.js` / `dist/green-radius.js`; `index.html` loads all nine `dist/` scripts in that order, then its `dist/src/boot-index.js` mount script — one of four small `src/boot-*.jsx` scripts (index/result/city/admin, also compiled to `dist/`) that define each page's palette and mount its root component. `result/`, `city/`, and `admin/` load only the subset they render plus their own `boot-*` script (`core` + `badge` everywhere, `share-card` on `result/`, `admin/aggregate.js` + `dist/admin/admin.js` on `admin/`) — `src/core.jsx` always first. Components reference each other by **bare name** within the shared global scope (see gotcha below).
